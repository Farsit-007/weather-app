 ...
 *
 * Everything under `extend` adds extra names on top of Tailwind's defaults, so
 * the components can write `text-sky-deep` or `shadow-md-soft` instead of
 * repeating raw colour codes. `content` tells Tailwind which files to scan for
 * class names.
 *
...
export default {
  ...
  theme: {
    extend: {
      // The app's colour palette, e.g. `bg-sky-soft`, `text-slate`.
      colors: {
        navy: {
          DEFAULT: "#13223f",
          soft: "#3d4f73",
      ...
      // Soft shadows used by the cards and buttons, e.g. `shadow-lg-soft`.
      boxShadow: {
      ...
      // Gradients. The `rec-*` ones are the suggestion card backgrounds and
      // match the types in utils/weatherRecommendation.js.
      backgroundImage: {
